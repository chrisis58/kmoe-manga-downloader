...
@unittest.skipUnless(
    KMOE_USERNAME and KMOE_PASSWORD,
    "KMOE_USERNAME and KMOE_PASSWORD must be set in environment variables",
)
class TestKmdrDownload(unittest.TestCase):
    ...
    def test_download_multiple_volumes(self):
        ...
        total_size = sum(
            ...
            for f in os.listdir(os.path.join(dest, book_dir))
            if os.path.isfile(os.path.join(dest, book_dir, f))
        )
