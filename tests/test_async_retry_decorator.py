...
class TestAsyncRetryDecorator(unittest.TestCase):
    ...
    def test_no_retry_on_non_retryable_status(self):
        mocked_404_func = AsyncMock(side_effect=aiohttp.ClientResponseError(history=(), request_info=MagicMock(), status=404, message="Not Found"))
